...
desc ddl_test; -- 테이블 이름은 ddlTest보다 ddl_test와 같은 형태를 선호
...
-- 서브쿼리를 이용해서 기존 테이블의 구조로 테이블 생성, 데이터도 저장, 제약조건은 복사되지 않음
create table emp02 
as 
select * from emp;
...
-- 테이블 변경 -> 저장 구조(스키마의 구조 : 컬럼들의 집합)가 변경되는 것
-- 컬럼 변경 : 테이블에 컬럼 추가, 테이블에 있는 컬럼 삭제, 컬럼의 내용이나 이름을 수정
...
-- 테이블 객체 삭제
-- drop table 테이블 이름 => 스키마 삭제와 저장 데이터의 삭제 
-- DDL : 자동 commit , DML 작업 중 DDL 작업은 신중하게★
...
-- 테이블의 모든 행을 삭제 : truncate table 체이블이름
select * from emp02;
...
-- not null 제약 조건 : 설정한 컬럼에 null 값 입력을 금지하는 제약 조건
-- 컬럼 수준에서만 정의 가능
drop table emp02;
-- 사원 테이블 생성 : 사원번호, 사원 이름, 직금, 부서명
-- 사원번호, 사원 이름에 not null 제약 조건 설정
create table emp02 (
...
-- default : 컬럼에 입력값이 없을 때 기본값을 성정해서 데이터 입력
